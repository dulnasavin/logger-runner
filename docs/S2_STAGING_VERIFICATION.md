...
4. Keep VPN2 as cold standby while VPN1 is healthy.
5. For a network failure, reconnect the current VPN once.
6. If VPN1 cannot recover, activate VPN2 once and verify the new route.
7. Recreate provider HTTP/SDK clients after a route change.
8. Do not switch VPN for authentication or provider server errors.
9. Enforce one global VPN2-switch budget for the logger run.
10. If both routes fail, fail closed and preserve production data.
